...
def run_bot(news_text):
    ...
    decision = generate_trade_signal(sentiment.lower(), score, positive_threshold=TRADE_THRESHOLD)
    logging.info(f"[ANALYSIS] {sentiment} @ {score:.2f} → {decision}")

    if decision in ["BUY", "SELL"]:
        client = get_alpaca_client()
        place_order(client, STOCK_SYMBOL, decision)
        logging.info(f"[TRADE] {decision} order placed for {STOCK_SYMBOL}.")
    else:
        logging.info("[TRADE] No trade executed.")
